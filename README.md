...
`FISCAL_START_MONTH = '2025-08'` なので、通期ビューと年収シミュレーションが集計するのは
**2025-08 〜 2026-07 の 12 ヶ月**。2026-08 以降の売上は通期の集計に入らない
（月別結果ビューでは通常どおり見られる）。第6期に入るときは
`src/constants/master.ts` の `FISCAL_START_MONTH` を更新すること。
