...
| 7 | 月次入力の共有 | `t_dept_inputs` シートに保存（解決済み） | 旧デプロイでは端末内に退避され警告が出る |
| 8 | 通期の範囲 | 第5期 = **2026-08 〜 2027-07**（確定済み） | 範囲外の月は通期に入らない（月別結果では見られる） |
...
- **7**: `t_dept_inputs`（経費・件数・目標・計画）と `t_monthly_notes`（会議メモ）を追加して解決した。
  シートは初回アクセス時に自動作成される。旧デプロイの GAS では従来どおり端末内に退避し、警告を出す。
- **8**: 確認して確定した（指示書に期首月の記載がなかった）。第6期に入るときは
  `src/constants/master.ts` の `FISCAL_START_MONTH` だけ更新すればよい。
